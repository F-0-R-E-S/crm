...
function useChart(canvasRef: React.RefObject<HTMLCanvasElement>, draw: (ctx: CanvasRenderingContext2D, w: number, h: number) => void) {
  useEffect(() => {
...
function LineChart({ colors, labels, datasets }: {
  colors: string[]
  labels: string[]
  datasets: number[][]
}) {
...
export default function AnalyticsPage() {
  ...
  const leads30 = days30.map(() => 600 + Math.floor(Math.random() * 900))
  const convs30 = leads30.map(v => Math.floor(v * (0.08 + Math.random() * 0.07)))
    ...
    <div className="page-section">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-2)', marginTop: 2 }}>Last 30 days · refreshed 5m ago</p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn-ghost">7D</button>
          <button className="btn-primary" style={{ fontSize: 12, padding: '8px 16px' }}>30D</button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 24 }}>
        {[
          { icon: '📥', val: '28,441', label: 'Leads Received',  delta: '+18%', color: '#4facfe', up: true },
          { icon: '📤', val: '21,803', label: 'Leads Delivered', delta: '+22%', color: '#34d399', up: true },
          { icon: '💰', val: '3,214',  label: 'Conversions',     delta: '+9%',  color: '#a78bfa', up: true },
          { icon: '🛡',  val: '1,847',  label: 'Fraud Blocked',   delta: '-4%',  color: '#f87171', up: false },
        ].map(k => (
          <div key={k.label} className="kpi-card">
            ...
            <div style={{ width: 36, height: 36, borderRadius: 10, background: `${k.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, marginBottom: 12 }}>{k.icon}</div>
            <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: -1 }}>{k.val}</div>
            <div style={{ fontSize: 12, color: 'var(--text-2)', marginTop: 4 }}>{k.label}</div>
            <div style={{ position: 'absolute', top: 16, right: 16, fontSize: 11, fontWeight: 600, padding: '3px 8px', borderRadius: 999, background: k.up ? 'rgba(52,211,153,0.14)' : 'rgba(248,113,113,0.14)', color: k.up ? '#34d399' : '#f87171' }}>{k.delta}</div>
          </div>
        ))}
      </div>
      ...
      <div className="glass-card" style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          <div className="section-label" style={{ margin: 0 }}>Leads & Conversions — 30 Days</div>
          <div style={{ display: 'flex', gap: 16 }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-2)' }}>
              <span style={{ width: 10, height: 3, borderRadius: 2, background: '#4facfe', display: 'inline-block' }}></span> Leads
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-2)' }}>
              <span style={{ width: 10, height: 3, borderRadius: 2, background: '#a78bfa', display: 'inline-block' }}></span> Conversions
            </span>
          </div>
        </div>
        <div style={{ height: 220 }}>
          <LineChart colors={['#4facfe', '#a78bfa']} labels={days30} datasets={[leads30, convs30]} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          ...
          {COUNTRIES.map(c => (
            <div key={c.c} style={{ marginBottom: 14 }}>
              ...
              <div className="score-track">
                <div className="score-fill" style={{ width: `${c.pct * 4}%`, background: 'var(--grad-blue)' }} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          <table className="glass-table">
            <thead>
              <tr>
                <th>Broker</th><th>Leads</th><th>Conv%</th><th>Revenue</th>
              </tr>
            </thead>
            <tbody>
              {BROKERS_PERF.map(b => (
