...
## v2.0 S3 — Routing iREV parity (2026-04-22)

Brought the rotation/flow subsystem to iREV feature-parity without changing the graph-canvas concept. Three sprint tags — `v2.0.0-s3.1-routing-backend` (model + engine), `v2.0.0-s3.2-routing-canvas` (editors + simulator UI), `v2.0.0-s3.3-routing-tree` (tree-list surface + scenario E2E). Canonical spec at `docs/superpowers/specs/2026-04-22-routing-irev-parity-design.md`; plan at `docs/superpowers/plans/2026-04-22-routing-irev-parity-plan.md`.

- **PQL.** `src/server/routing/pql/` — 8-field registry (`geo, subId, utm_source, utm_medium, affiliateId, timeOfDay, phone, hourOfDay`), 10 signs, per-rule `caseSensitive` toggle. `evaluatePqlGate(rules, logic, lead, now?)` — pure, shortcircuits AND, returns `{ok, failedRuleIndex, failedField, failedSign}`.
- **New Zod nodes** in `src/server/routing/flow/model.ts`: `SmartPool` (priority-failover container) + `ComparingSplit` (A/B) + `PqlGate` on `BrokerTarget` + `active`/`description` on `BrokerTarget`. Filter `conditions` → `rules` renamed; legacy input still parsed by `FlowGraphSchema.preprocess` (`preprocessFlowGraphInput`). Data migration: `scripts/migrate-filter-to-pql.ts` (idempotent; `--dry-run`).
- **Publish-time compilation.** `src/server/routing/flow/compile-smartpool.ts` translates a SmartPool + ordered children into `FallbackStep` rows; `compile-comparing.ts` translates a ComparingSplit + shares into `Algorithm(WRR)` + `FlowAlgorithmConfig` payload. `publishFlow` calls the compiler, merges pool-derived rows with author-authored rows (pool-derived wins on `(fromNodeId, hopOrder)` collision), runs the existing `detectFallbackCycle` on the merged set.
- **Engine** (`src/server/routing/engine.ts`). After cap check, BEFORE algorithm pick: per-target `active` filter + per-target `pqlGate` evaluation (miss → excluded from `available`, traced as `pql_gate` step). SmartPool bias: if any available target is a pool child, pick the highest-rank available one — no algorithm call. WRR / Slots-Chance only run when no pool applies. Decision surfaces `selectedSmartPoolId` + `selectedComparingSplitId`.
- **Cap engine** (`src/server/routing/constraints/caps.ts`). `kind: "PUSHED" | "REJECTED"` threaded through `consumeCap` / `releaseCap` / `remainingCap`. `pqlScope` gates consumption (miss → skipped bypass); bucket key is salted by `sha256(pqlScope).slice(0,8)` so distinct scopes on the same broker don't collide. `effectiveRejectedLimit(limit, rejectedLimit, asPercent)` exposed.
- **Rejection streak** — `src/server/routing/constraints/rejection-streak.ts`. Redis key `cap:streak:{brokerId}:{flowVersionId}` with 24h TTL. `bumpRejectionStreak` / `resetRejectionStreak` / `readRejectionStreak` / `shouldAutoPause(count, threshold)`. Postback handler (`src/app/api/v1/postbacks/[brokerId]/route.ts`) bumps on `DECLINED`, resets on `ACCEPTED`/`FTD`; on hitting `min(rejectionsInARow)` across the broker's `CapDefinition` rows, flips `Broker.isActive=false`, emits `BROKER_REJECTION_STREAK_PAUSED` (admin-only Telegram), writes AuditLog. Scope key `"global"` because `Lead` doesn't carry flowVersionId yet.
- **Simulator.** `simulateBatch` in `src/server/routing/simulator.ts` — runs N leads through `executeFlow` in dryRun, rolls per-broker acceptance, walks `FallbackStep` chain on reject (pool children only). REST `/api/v1/routing/simulate-pool` exposes it synchronously. UI: `/dashboard/routing/flows/:id/simulator` gains a `pool` tab with per-broker accept-probability JSON editor + per-broker tallies + first-10 sequential traces.
- **Canvas.** `SmartPoolNode` + `ComparingSplitNode` renderers in `src/components/routing-editor/nodes.tsx`; registered in `Canvas.nodeTypes` under keys `smartPool` / `comparingSplit`. Toolbar gets `+ SmartPool` + `+ Compare` buttons; Fallback marked deprecated (not yet migrated). BrokerTarget inspector section: `active` checkbox + `description` field + optional PQL gate (reuses `FilterConditionEditor`). CapInspector: rejected-capacity number, asPercent checkbox, rejectionsInARow number.
- **Filter editor vocabulary.** `src/components/routing-editor/filter-conditions.ts` — full 8×10 matrix; per-row `caseSensitive?: boolean`. Editor (`FilterConditionEditor.tsx`) adds per-row `Aa` checkbox. Inspector adapter (`Inspector.tsx`) bridges between editor's legacy-shape `{field, op, value, caseSensitive}` and the canonical `PqlRule.{field, sign, value, caseSensitive}`.
- **Tree-list surface.** `src/server/routing/flow/tree.ts::flowToTree(graph)` folds a FlowGraph into `TreeRoot` + folder/leaf shapes. `summarizeTree` gives a deterministic snapshot string for tests. tRPC `routing.treeView`. Page `/dashboard/routing/flows/:id/tree` — read-only compact view with kind Pills (AD/FLT/ALG/SM/CO/FB), expand-collapse per folder, quick-switch "Tree" button in canvas header. Inline edit/create deferred.
- **Prisma schema** (`20260424120000_v2_s3_1_routing_irev_parity`). Additions: `CapDefinition.{rejectedLimit, rejectedLimitAsPercent, rejectionsInARow, pqlScope, behaviorPattern}`, `CapCounter.kind` (PUSHED/REJECTED) + unique-index swap, new `ComparingBucketStat` table + back-relation on FlowVersion, new enums `CapBehaviorPattern` / `CapCounterKind`. `resetDb` unchanged (new table cascades).
- **Telegram.** New admin-only event `BROKER_REJECTION_STREAK_PAUSED` + template; event catalog + `ADMIN_ONLY_EVENTS` + templates/index updated (enforced by `tests/unit/telegram-templates.test.ts`).
- **Scenario gate.** `tests/e2e/routing-irev-scenario.test.ts` — authors a `Filter(geo=UA) → SmartPool → {b1, b2, b3}` flow via `createDraftFlow`, publishes (asserts 2 FallbackStep rows emitted), runs `simulateBatch({count:100, probs:{b1:0, b2:0, b3:1}})` — every lead MUST land on b3 via a 3-hop trace. Companion `[1,0,0]` test asserts single-hop.
- **Test count.** ~65 net-new tests, 969 total. Flaky `tests/integration/telegram-events-wired.test.ts` still in parking lot (known pre-existing).
- **Release.** `package.json` bumped to `2.0.0-s3.3`. Three tags on `main`.
