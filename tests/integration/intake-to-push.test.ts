...
import { startBossOnce, JOB_NAMES } from "@/server/jobs/queue";
...
describe("intake → worker", () => {
  ...
  beforeEach(async () => {
    ...
    await prisma.apiKey.create({ data: { affiliateId: aff.id, keyHash: sha(rawKey), keyPrefix: rawKey.slice(0, 12), label: "f" } });
  ...
  afterEach(() => mb.stop());
  afterAll(async () => { const boss = await startBossOnce(); await boss.stop({ graceful: true, timeout: 1000 }); });

  it("end-to-end PUSHED state", async () => {
    mb.respondWith(200, { id: "e2e-ext-1", status: "accepted" });
    const res = await POST(new Request("http://x/api/v1/leads", {
      method: "POST",
      headers: { "content-type": "application/json", authorization: `Bearer ${rawKey}` },
      body: JSON.stringify({ geo: "UA", ip: "1.1.1.1", firstName: "E2E", email: "e2e@x.com", event_ts: new Date().toISOString() }),
    }));
