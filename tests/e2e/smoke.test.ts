...
describe("E2E smoke — happy path", () => {
  let mb: MockBroker;
  let tr: MockTracker;
  let rawKey = "ak_e2e_" + "x".repeat(40);
  let brokerId = "";
  ...
  beforeAll(async () => {
    ...
    await prisma.apiKey.create({ data: { affiliateId: aff.id, keyHash: sha(rawKey), keyPrefix: rawKey.slice(0, 12), label: "e2e" } });
  ...
  afterAll(async () => { await mb.stop(); await tr.stop(); });

  it("intake → push → FTD via postback → outbound tracker", async () => {
    // 1. intake
    mb.respondWith(200, { id: "e2e-ext-1", status: "accepted" });
    const intakeRes = await INTAKE(new Request("http://x/api/v1/leads", {
      method: "POST",
      headers: { "content-type": "application/json", authorization: `Bearer ${rawKey}` },
      body: JSON.stringify({ geo: "UA", ip: "1.1.1.1", firstName: "E2E", email: "e2e@x.com", phone: "0671234567", sub_id: "sub-e2e", event_ts: new Date().toISOString() }),
    }));
    ...
    expect(tr.hits.some((h) => h.url.includes("click=sub-e2e") && h.url.includes("s=lead_pushed"))).toBe(true);

    // 4. inbound postback: FTD
    const pbBody = { lead_id: "e2e-ext-1", status: "ftd" };
    const pbSig = signHmac(brokerSecret, JSON.stringify(pbBody));
    const pbRes = await POSTBACK(new Request(`http://x/api/v1/postbacks/${brokerId}`, {
      method: "POST",
