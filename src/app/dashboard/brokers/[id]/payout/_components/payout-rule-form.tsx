...
export function PayoutRuleForm({
  brokerId,
  onSaved,
}: {
  brokerId: string;
  ...
  return (
    ...
    >
      ...
      {(kind === "CPA_FIXED" || kind === "CPA_CRG" || kind === "HYBRID") && (
        <label>
          <span style={labelStyle}>Min qualified deposit (USD)</span>
          <input
            type="number"
            step="0.01"
            value={minQualifiedDeposit}
            onChange={(e) => setMinQualifiedDeposit(e.target.value)}
            style={fieldStyle}
          />
        </label>
      )}
