...
export default function BrokerPayoutPage() {
	...
	const rules = trpc.finance.listBrokerPayoutRules.useQuery(
		{ brokerId: brokerId ?? "" },
		{ enabled: !!brokerId },
	);
	...
	return (
		...
		>
			<div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
				...
				<Link
					href={`/dashboard/brokers/${brokerId}`}
					style={{
						fontSize: 11,
						fontFamily: "var(--mono)",
						color: "var(--fg-1)",
			...
			<section>
				...
				<table style={{ width: "100%", borderCollapse: "collapse" }}>
					<thead>
						...
						>
							<th style={{ padding: "6px 0" }}>Kind</th>
							<th>CPA</th>
							<th>CRG</th>
							<th>Rev share</th>
							<th>From</th>
					...
					<tbody>
						{rules.data?.map((r) => (
							<tr key={r.id} style={{ borderBottom: "1px solid var(--bd-1)" }}>
								<td style={{ padding: "6px 0", fontFamily: "var(--mono)" }}>{r.kind}</td>
								<td style={{ fontFamily: "var(--mono)" }}>
									{r.cpaAmount?.toString() ?? "—"}
								</td>
								<td style={{ fontFamily: "var(--mono)" }}>
									{r.crgRate?.toString() ?? "—"}
								</td>
								<td style={{ fontFamily: "var(--mono)" }}>
									{r.revShareRate?.toString() ?? "—"}
								</td>
								<td style={{ fontFamily: "var(--mono)" }}>
								...
								<td style={{ fontFamily: "var(--mono)" }}>
									{r.activeTo
										? new Date(r.activeTo).toISOString().slice(0, 10)
										: "—"}
								</td>
							</tr>
						))}
			...
			<section style={{ maxWidth: 480 }}>
				...
				<PayoutRuleForm
					brokerId={brokerId}
					onSaved={() =>
						utils.finance.listBrokerPayoutRules.invalidate({ brokerId })
					}
				/>
			</section>
		</div>
