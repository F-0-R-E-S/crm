...
describe("renderPostbackUrl", () => {
  it("substitutes {sub_id} {status} {payout}", () => {
    ...
    const out = renderPostbackUrl(url, {
      sub_id: "abc",
      status: "ftd",
      payout: "50",
      lead_id: "l",
  ...
  it("URL-encodes special chars", () => {
    const out = renderPostbackUrl("http://t?x={status}", {
      sub_id: "",
      status: "a b & c",
      payout: "0",
      lead_id: "",
  ...
  it("leaves unknown macros as literal", () => {
    const out = renderPostbackUrl("http://t?x={unknown}", {
      sub_id: "",
      status: "",
      payout: "",
      lead_id: "",
  ...
  it("empty sub_id renders as empty string", () => {
    const out = renderPostbackUrl("http://t?c={sub_id}", {
      sub_id: "",
      status: "",
      payout: "",
      lead_id: "",
