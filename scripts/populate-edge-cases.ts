...
const GEO_WEIGHTS: Array<[string, number]> = [
  ["DE", 22], ["FR", 18], ["IT", 12], ["ES", 10], ["PL", 10],
  ["NL", 8], ["PT", 6], ["AT", 5], ["CH", 3], ["BE", 3], ["IE", 2], ["FI", 1],
];
...
const FIRST_NAMES = [
  "Alex","Maria","Luca","Sofia","Max","Emma","Paul","Anna","Leo","Julia","Tom","Laura","Ben","Clara",
  "Nico","Lena","Finn","Nina","Hugo","Elena","Jan","Alice","Oskar","Eric","Chiara","Kai","Noa","Diego",
  "Olivia","Pavel","Iryna","Piotr","Zofia","Sven","Katja","Marco","Beatriz",
];
const LAST_NAMES = [
  "Muller","Rossi","Garcia","Dubois","Kowalski","Janssen","Silva","Bauer","Bianchi","Lopez","Martin",
  "Nowak","DeJong","Costa","Huber","Russo","Fernandez","Bernard","Wozniak","Peters","Almeida","Gruber",
  "Ricci","Sanchez","Petit","Kaminski","Visser","Santos","Wagner","Ferrari",
];
const SUB_IDS = ["fb-crypto-eu-q1","fb-forex-eu-q1","gg-crypto-dach","gg-forex-pl","tg-crypto-main","native-eu-push","tiktok-finance","yt-crypto-edu"];
const UTM_SOURCES = ["facebook","google","telegram","native","tiktok","youtube"];
const UTM_MEDIUMS = ["cpc","cpm","organic","paid_social"];
const UTM_CAMPAIGNS = ["q1-crypto-eu","q1-forex-dach","q1-push-eu","eu-retarget","spring-launch","q2-bull-run"];
const LANDING_PAGES = ["/land/crypto-starter","/land/forex-pro","/land/trading-now","/land/bull-run","/land/smart-invest","/land/free-signals"];
const EMAIL_DOMAINS = ["gmail.com","outlook.com","yahoo.com","proton.me","mail.ru","web.de","libero.it","orange.fr"];

function pick<T>(a: readonly T[]): T { return a[randomInt(a.length)]; }
function digits(n: number): string { let s=""; for (let i=0;i<n;i++) s += String(randomInt(10)); return s; }
function sleep(ms: number): Promise<void> { return new Promise((r) => setTimeout(r, ms)); }
function randomIp(): string { return `${randomInt(1, 224)}.${randomInt(256)}.${randomInt(256)}.${randomInt(1, 255)}`; }
function ascii(s: string): string { return s.normalize("NFD").replace(/[\u0300-\u036f]/g, ""); }
function pickGeo(): string {
  const total = GEO_WEIGHTS.reduce((a, [_, w]) => a + w, 0);
  const r = randomInt(total);
  let acc = 0;
  for (const [g, w] of GEO_WEIGHTS) { acc += w; if (r < acc) return g; }
  return GEO_WEIGHTS[0][0];
}
function validPhone(geo: string): string {
  const gen = PHONE_GEN[geo]; if (!gen) return `+49170${digits(7)}`;
  for (let i = 0; i < 8; i++) {
    const raw = gen();
...
// --- admin session ---
function curlCmd(args: string[]): string { return execFileSync("curl", ["-sS", ...args], { encoding: "utf8" }); }
function loginAdmin(): string {
  ...
  try {
    ...
    curlCmd([
      "-c", jarPath, "-b", jarPath, "-X", "POST",
      "-H", "content-type: application/x-www-form-urlencoded",
      "--data-binary",
      new URLSearchParams({ csrfToken, email: EMAIL, password: PASSWORD, callbackUrl: BASE, json: "true" }).toString(),
  ...
  return createTRPCClient<AppRouter>({
    links: [
      httpBatchLink({
        ...
        fetch: (url, opts) => fetch(url as string, { ...(opts ?? {}), headers: { ...(opts?.headers ?? {}), cookie } }),
      }),
    ],
  });
}
...
// --- HTTP helpers ---
interface IntakeResult { lead_id: string; status: string; reject_reason: string | null; trace_id: string; received_at: string; }
...
async function phaseRejected(cookie: string) {
  ...
  const recent = await trpc.lead.list.query({ state: "ACCEPTED", page: 1, pageSize: Math.min(DEDUP_COUNT + 20, 500) });
...
async function phaseFreshBurst(): Promise<Array<{ id: string }>> {
  ...
  for (let i = 0; i < FRESH_COUNT; i++) {
    ...
    if ((i + 1) % 50 === 0) {
      ...
      console.log(`  fresh ${i + 1}/${FRESH_COUNT} ok=${accepted.length} rej=${rejected} fail=${failed} elapsed=${el.toFixed(0)}s`);
    }
  }
...
async function phaseFreshPostback(accepted: Array<{ id: string }>) {
  ...
  const acc = shuffled.slice(ftd.length, ftd.length + Math.round(n * 0.20));
  const dec = shuffled.slice(ftd.length + acc.length, ftd.length + acc.length + Math.round(n * 0.10));
  console.log(`  postback plan (of ${n}): ftd=${ftd.length} accepted=${acc.length} declined=${dec.length} stay_pushed=${n - ftd.length - acc.length - dec.length}`);
